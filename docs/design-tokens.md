...
| 出口 | 在哪 | 用在哪 |
|---|---|---|
| 对照卡的**卡头铺色** | `comparison-card.tsx` | 结果页、首页、`/sample`。结论就是那张卡的脸 |
| `VerdictBadge` | `verdict.tsx` | 图例、教学位（首页「只判三种结果」那三枚）。`tone="dark"` 给墨黑面 |

> 上一版还有一枚 `VerdictSeal`（页边印章），随「840px 阅读栏 + 7rem 页边」那套布局
> 一起退场了 —— 逐笔改成横向卡带之后没有页边可盖，而卡头已经整片铺成结论色，
> 再盖一枚章就是同一个结论说两遍。
...
| 三 · 拿去发 | 申诉信 + 中文对照 + 邮件话术 | 单列 840px：信是成品（「复制全文」坐在卡头上），底下跟两条**默认折叠**的附件 |
